...
🖼️ Fotoğraf - Sadece Instagram'da çalişir
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add photo option only for Instagram
    if platform == 'instagram':
        keyboard.append([
            InlineKeyboardButton("🖼️ Fotoğraf (Sadece Instagram)", callback_data="photo_only"),
        ])
...
async def download_photo(query, context, url, chat_id):
    ...
    try:
        ...
        if 'Unsupported URL' in error_msg or 'not supported' in error_msg.lower():
            await query.edit_message_text(
                "⚠️ <b>Fotoğraf İndirilemedi!</b>\n\n"
                "Bu platformda fotoğraf indirme desteklenmiyor.\n\n"
                "📌 <b>Alternatif:</b>\n"
                "• Video olarak indirmeyi deneyin\n"
                "• Instagram fotoğraf linki kullanin\n\n"
                "🖼️ Fotoğraf özelliği sadece Instagram'da çalışır.",
                parse_mode='HTML'
            )
        elif 'is a playlist' in error_msg.lower():
            await query.edit_message_text(
